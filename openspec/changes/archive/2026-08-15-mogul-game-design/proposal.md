...
- **Theater network**: fictional 1930s country map, theaters as city slots (single screen → palace), distribution costs as connections, film exchanges as distribution networks.
...
- `game-core`: turn structure, the five phases, money loop, income table, era clock, victory condition (most theaters lit, ties on money then theaters built).
...
- `game-lobby`: rooms, invitations, seat management, mixed human/AI seats, bot handoff for dropped players, reconnection.
- `game-ai`: scripted bot strategy and the agent CLI client, including the player protocol (state snapshot in, moves out), the turn-loop action menu, move clocks, and validation.
- `game-client`: minimal browser client — snapshot rendering, command submission, reconnect.
